...
# ══════════════════════════════════════════════════════════════════════════════
#  FUNGSI PERKALIAN MATRIKS — STRASSEN
#  Menggantikan operator @ / np.matmul / np.dot dari versi numpy
#  Kompleksitas: O(n^log₂7) ≈ O(n^2.807)  — Divide and Conquer
#  Relasi Rekurens: T(n) = 7·T(n/2) + O(n²)  →  Master Theorem Case 1
# ══════════════════════════════════════════════════════════════════════════════

# Threshold base case: ukuran n ≤ ini langsung pakai np.dot
_STRASSEN_BASE = 8
...
def matmul_brute_force(A, B):
    ...
    Algoritma (Strassen, 1969):
        Bagi A dan B menjadi 4 sub-matriks n/2 × n/2, lalu hitung
        7 produk bantu M1–M7 (bukan 8 seperti naive) secara rekursif:
    ...
    Kompleksitas    : O(n^log₂7) ≈ O(n^2.807)  [Master Theorem Case 1]
...
def _strassen_pad(A, B):
    """Pad ke persegi power-of-2, jalankan rekursi, potong ke ukuran asli."""
    r, k  = A.shape
    k2, c = B.shape
    # Cari ukuran persegi power-of-2 terkecil yang menampung semua dimensi
    n = 1
    while n < max(r, k, c):
        n *= 2
...
def _strassen_rec(A, B):
    ...
    n = A.shape[0]
    # Base case: overhead rekursi lebih mahal untuk ukuran kecil
    if n <= _STRASSEN_BASE:
        return A @ B
    # Pad ke genap jika n ganjil
    padded = (n % 2 != 0)
    if padded:
        A = np.pad(A, ((0, 1), (0, 1)))
        B = np.pad(B, ((0, 1), (0, 1)))
...
def _warp_affine_brute(img, M2x3):
    ...
    Perkalian matriks M @ [x, y, 1]^T dilakukan dengan matmul_brute_force (Strassen).
...
def _warp_perspective_brute(img, M3x3):
    ...
    Perkalian matriks M_inv @ [x, y, 1]^T memakai matmul_brute_force (Strassen).
...
def bab5_domain_frekuensi(gray):
    ...
    # --- 5c. DCT 2D — menggunakan matmul_brute_force (Strassen) ---
    ...
    for y in range(0, H8, 8):
        for x in range(0, W8, 8):
            ...
            # DCT 2D: D @ blok @ D.T  — dua kali matmul_brute_force (Strassen)
            temp = matmul_brute_force(D8,  blok)   # O(8^2.807) Strassen
            Dblk = matmul_brute_force(temp, D8T)   # O(8^2.807) Strassen
            ...
            # IDCT 2D: D.T @ Dq @ D  — dua kali matmul_brute_force (Strassen)
            temp2 = matmul_brute_force(D8T, Dq)   # O(8^2.807) Strassen
            R     = matmul_brute_force(temp2, D8)  # O(8^2.807) Strassen
...
def bab8_transformasi_geometri(img_bgr):
    ...
    Perkalian matriks affine/perspektif menggunakan matmul_brute_force (Strassen).
    """
    ...
    M_trans = np.float64([[1, 0, tx],
                           [0, 1, ty]])
    # Translasi menggunakan inverse mapping + matmul_brute_force (Strassen)
    ...
    # Perspektif: perkalian matriks 3×3 × [x,y,1]^T via matmul_brute_force (Strassen)
...
def bab10_kompresi(gray):
    ...
    Perkalian matriks DCT menggunakan matmul_brute_force (Strassen).
    """
    ...
    # --- 10b. JPEG-like lossy compression — DCT via matmul_brute_force (Strassen) ---
    Q_luma = np.array([
